# bf16 e2e test (Step D in bf16): the FULL one-stage fused kernel, end-to-end, on 2 GPUs.
...
in_t = nvt.tensor((MAXTOK, K), dtype=torch.bfloat16)
out_t = nvt.tensor((MAX_SLOTS, K), dtype=torch.bfloat16)  # inbox == gemm A
...
# nvshmem cute RMA has no bf16 entry in its dtype table. Dispatch is byte movement
# (ref uses byte-count putmem_warp), so hand it a fp16 VIEW of the same bf16 bytes —
# identical bytes move; the gemm reads the buffer as bf16 via a_base_ptrs, the decode
# reads out_t as bf16. (16-bit view => same element count/byte size.)
...
for g, (rb, m) in enumerate(zip(row_begins, Ms)):
    ...
    b_ref = mega_kernel._to_reference_operand_fp32(bc_tensors[g][1], cutlass.BFloat16)
